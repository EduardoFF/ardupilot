...
*/
AP_RangeFinder_trone::AP_RangeFinder_trone(RangeFinder &_ranger, uint8_t instance, RangeFinder::RangeFinder_State &_state)
    : AP_RangeFinder_Backend(_ranger, instance, _state)
    , dev(hal.i2c_mgr->get_device(1, TRONE_I2C_ADDR))
{
}
...
AP_RangeFinder_Backend *AP_RangeFinder_trone::detect(RangeFinder &ranger, uint8_t instance, RangeFinder::RangeFinder_State &_state)

{
    AP_RangeFinder_trone *sensor = new AP_RangeFinder_trone(ranger, instance, _state);
    if (!sensor) {
        return nullptr;
    }
...
bool AP_RangeFinder_trone::init(void)
{
    ...
    dev->register_periodic_callback((uint32_t)50000,
                                    FUNCTOR_BIND_MEMBER(&AP_RangeFinder_trone::timer, bool));
...
*/
bool AP_RangeFinder_trone::timer(void)
{
    // take a reading
    uint16_t distance_cm;
